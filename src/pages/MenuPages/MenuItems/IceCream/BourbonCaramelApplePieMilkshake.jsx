...
const BourbonCaramelApplePieMilkshake = () => {
  const iceCreamData = menuData.filter((item) => item.category === "Ice Cream");
  const iceCreamItems = iceCreamData.map((item) => item.featuredItems);
  const iceCream = iceCreamItems[0].filter(
    (item) => item.name === "Bourbon Caramel Apple Pie Milkshake"
  )[0];
  return (
    <>
      <MenuItem
...
export default BourbonCaramelApplePieMilkshake;
